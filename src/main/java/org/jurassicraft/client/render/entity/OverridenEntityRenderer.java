...
public class OverridenEntityRenderer extends EntityRenderer {
	...
	private final float maxThirdPersonDistance = 30;
	private final float minThirdPersonDistance = 4.0F;
	private float thirdPersonDistance = 4.0F;
	/** Previous third person distance */
	private float thirdPersonDistancePrev = this.thirdPersonDistance;
	/** Smooth cam yaw */
	...
	public OverridenEntityRenderer(Minecraft mcIn, IResourceManager resourceManagerIn) {
	...
	public void updateRenderer() {
		...
		this.thirdPersonDistancePrev = this.thirdPersonDistance;
	...
	public void updateCameraAndRender(float partialTicks, long nanoTime) {
		...
		if (!this.mc.skipRenderWorld) {
			...
			if (this.mc.world != null) {
				...
				// Forge: also fix rendering text before entering world (not part of MC-112292,
				// but the same reason)
				...
				try {
					...
					crashreportcategory.addDetail("Screen name", new ICrashReportDetail<String>() {
						public String call() throws Exception {
							return OverridenEntityRenderer.this.mc.currentScreen.getClass().getCanonicalName();
						}
					});
	...
	private void renderWorldPass(int pass, float partialTicks, long finishTimeNano) {
		...
		ActiveRenderInfo.updateRenderInfo(this.mc.getRenderViewEntity(), this.mc.gameSettings.thirdPersonView == 2); // Forge: MC-46445
																														// Spectator mode
																														// particles and
																														// sounds computed
																														// from where you
